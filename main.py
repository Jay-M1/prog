...
def main():
    ...
    sb_points = (Vector(100,700), Vector(10,700), Vector(10,710), Vector(100,710))
    starter_bat = Bat(screen, colors['red'], sb_points, right=True, anschlag=10) # setzt den Anschlag des Schlägers fest
    starts = 5
    ...
    while running:
        ...
        if ball1.check_collision(big_ball) == None: 
            pass
        else:
            ball2_here = True
            ball2_time_begin = pygame.time.get_ticks()

        if pygame.time.get_ticks() - ball2_time_begin > 20000: ball2_here = ball1.check_collision(big_ball)
        ...
        for ball in balls:
            if ball.velocity.abs() < 1 * fps_multiplyer:
                ...
                if time % 5000 <= 1: score -= 1 # wenn 5 sekunde ohne bewegung vergeht, dann gibts punkteabzug. 
            else:
                time = 1
        ...
        if da:
            ...
            if highscore > 0:
                ...
                your_highscore = test_font.render(f'Your Highscore: noch keins', False, 'White')
        ...
        # Motion
        if ball2_here: ball1.check_collision(ball2)
        if ball2_here: ball2.check_collision(big_ball2)
        spawn2 = ball1.check_collision(big_ball2)
        if ball2_here: ball2.check_collision(big_ball)
        ball1.gravitate()
        if ball2_here: ball2.gravitate()
        # key_left = left_bat.update_left(key_left)
        # key_right = right_bat.update_right(key_right)

        for ball in balls:          # die Schleife checkt, ob ein Ball in die "Aus" Zone kommt
        ...
        clock.tick(200 * fps_multiplyer) # 60 frames per second        
...
# fliperrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrhebel